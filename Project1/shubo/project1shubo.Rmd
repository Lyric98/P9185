...
endpoints$period1.adverse = as.numeric((endpoints$AE_pillA_week1 + endpoints$AE_pillA_week2 + endpoints$AE_pillA_week3 + endpoints$AE_pillA_week4) != 0)
endpoints$period2.adverse = as.numeric((endpoints$AE_gelB_week1 + endpoints$AE_gelB_week2 + endpoints$AE_gelB_week3 + endpoints$AE_gelB_week4) != 0)
endpoints$period3.adverse = as.numeric((endpoints$AE_gelC_week1 + endpoints$AE_gelC_week2 + endpoints$AE_gelC_week3 + endpoints$AE_gelC_week4) != 0)
...
kableExtra::kable(round(cbind(QIC1, QIC2, QIC3), 2), align = c("c", "c", "c", "c", "c", "c"),
      caption = "QIC table")
## mod1.1 will be the best model
summary(mod1.1$model)
...
```{r, #data}
endpoints$period1.adherence = as.numeric((endpoints$Adhere_pillA_week1 + endpoints$Adhere_pillA_week2 + endpoints$Adhere_pillA_week3 + endpoints$Adhere_pillA_week4))
endpoints$period2.adherence = as.numeric((endpoints$Adhere_gelB_week1 + endpoints$Adhere_gelB_week2 + endpoints$Adhere_gelB_week3 + endpoints$Adhere_gelB_week4))
endpoints$period3.adherence = as.numeric((endpoints$Adhere_gelC_week1 + endpoints$Adhere_gelC_week2 + endpoints$Adhere_gelC_week3 + endpoints$Adhere_gelC_week4))

obj2 <- endpoints[,c(1:4,17:31)]
temp1 <- melt(obj2, id.vars = c("ptid", "age", "race", "gender"), measure.vars = c("period1", "period2", "period3"))
temp2 <- melt(obj2, id.vars = c("ptid", "age", "race", "gender"), measure.vars = colnames(obj2)[5:16])
temp2$Period=NA    
temp2$Treatment=NA
colnm <- colnames(temp1)
colnm[c(5,6)] <- c("Period", "Treatment")
...
df$Treatment <- as.factor(substr(df$Treatment, nchar(df$Treatment), nchar(df$Treatment)))
df$Treatment <- relevel(df$Treatment, ref = "B")
df$ptid <- as.numeric(df$ptid)


carryover2<-createCarry(data=df, treatment="Treatment",id="ptid", period ="Period",carrySimple=TRUE)
colnames(carryover2$data)[c(5,9,10)] <- c("Time", "Carry_C", "Carry_A")
...
plot(df$y ~ as.numeric(df$Time))

mod2.0 <- glm(cbind(y, 7-y) ~ 1, family = binomial, data = df)
n <- nrow(df)
r221 <- as.numeric(1 - exp(-2 / n * (logLik(mod2.1) - logLik(mod2.0))))

mod2.1 <- glm(cbind(y, 7-y) ~ age + gender + race + Period + Time + Treatment + Carry_C + Carry_A, 
            family = binomial, data = df)
res2.1 <- summary(mod2.1)$coef
aicbic2.1 <- c(AIC(mod2.1), BIC(mod2.1))


mod2.2<- glmer(cbind(y, 7-y) ~  Period + Time + Treatment + Carry_C + Carry_A + (1|ptid), data = df, family = binomial, nAGQ = 5)
res2.2 <- summary(mod2.2)$coef
aicbic2.2 <- c(AIC(mod2.2), BIC(mod2.2))
n <- nrow(df)
r222 <- as.numeric(1 - exp(-2 / n * (logLik(mod2.2) - logLik(mod2.0))))

mod2.3<- glmer(cbind(y, 7-y) ~ age + gender + race + Period + Time + Treatment + Carry_C + Carry_A + (1 + Time |ptid), data = df, family = binomial)

res2.3 <- summary(mod2.3)$coef
aicbic2.3 <- c(AIC(mod2.3), BIC(mod2.3))
n <- nrow(df)
r223 <- as.numeric(1 - exp(-2 / n * (logLik(mod2.3) - logLik(mod2.0))))

compare.matrix <- cbind(rbind(aicbic2.1, aicbic2.2, aicbic2.3), rbind(r221, r222, r223))
colnames(compare.matrix) <- c("AIC", "BIC", "R square")
rownames(compare.matrix) <- c("ModelA", "ModelB", "ModelC")
table <- kableExtra::kable(compare.matrix, align = c("c", "c", "c"),
...
fin_tbl <- round(cbind(beta_est, 
                 beta_est - 1.96 * sqrt(diag(beta_var)), 
                 beta_est + 1.96 * sqrt(diag(beta_var))), digits = 3)
colnames(fin_tbl) <- c("Est", "95% CI LB", "95% CI UB")
kableExtra::kable(round(fin_tbl, 3), align = c("c", "c", "c"),
      caption = "Summary table")
### to see which contains 0 thus is insignificant

fin_tbl_exp <- round(cbind(exp(beta_est), 
                 exp(beta_est - 1.96 * sqrt(diag(beta_var))), 
                 exp(beta_est + 1.96 * sqrt(diag(beta_var)))), digits = 3)
colnames(fin_tbl_exp) <- c("Est", "95% CI LB", "95% CI UB")
kableExtra::kable(round(fin_tbl_exp, 3), align = c("c", "c", "c"),
...
mod3.1 = lm(y ~ age + gender + race + Pos + Treatment + Period + Carry_B + Carry_C + Adhere + Adverse, data = obj3)
result_summary = generate_result_summary(mod3.1, "lm", obj3)
...
mod3.2<- lmer(y ~ age + gender + race + Pos + Treatment + Period + Carry_B + Carry_C + Adhere + Adverse + (1|ptid), data = obj3)
result_summary = generate_result_summary(mod3.2, "lmm", obj3)
kableExtra::kable(data.frame(cbind(result_summary$res_table,p_value=round(result_summary$sand_coef_test$p_t,3))))
metrics[,2]=result_summary$metrics

mod3.3<- lmer(y ~ age + gender + race + Pos + Treatment + Period + Carry_B + Carry_C + Adhere + Adverse + (1 + Pos |ptid), data = obj3)
result_summary = generate_result_summary(mod3.3, "lmm", obj3)
kableExtra::kable(data.frame(cbind(result_summary$res_table,p_value=round(result_summary$sand_coef_test$p_t,3))))
metrics[,3]=result_summary$metrics

metrics=data.frame("Model 1"=metrics[,1], "Model 2"=metrics[,2], "Model 3"=metrics[,3],row.names = c("AIC","BIC","R2"))
kableExtra::kable(metrics)
### mod3.2 will be the best
#kableExtra::kable(Anova(mod3.2, type="III",test="F"))
# refactor (A as reference)
result_summary = generate_result_summary(mod3.2, "lmm", obj3)
kableExtra::kable(result_summary)
