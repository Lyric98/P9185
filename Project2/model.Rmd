...
temp <- data %>% 
  group_by(subject_id) %>%
  mutate(mem_diff = mem_comp - mem_comp[1])
temp <- temp %>%
  select(-mem_comp) %>%
  slice(2:n())
temp$gender <- as.factor(temp$gender)
temp$treatment_group <- as.factor(temp$treatment_group)

model <- with(temp, lmer(mem_diff ~ age + gender + treatment_group+day + (1|subject_id)))
...
model <- with(temp, lmer(mem_diff ~ age + gender + treatment_group+day  +treatment_group:day + (1|subject_id)))
